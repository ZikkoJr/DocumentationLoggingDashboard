...
> **Arrival Month correction hold (2026-07-23):** The prior candidate at `3842298` is the starting source baseline, not evidence for the new rule. Do not use the frozen pilot package, Retry-01 root, or prior production-candidate package to validate this correction. Initial focused tests and representative PDF/save/index evidence passed, but reviewer-expanded Debug/Release and V1 executable reruns are blocked by Windows Smart App Control. A corrected package and pilot resumption require completed evidence, an authorized commit, independent review, and separate pilot authorization. See `Pilot-Correction-Arrival-Month-Threshold.md`.
...
| P-13 | Hotel A / PMS 1 | One monetary column | Arrival threshold sequence | Using valid nonblank Arrival Dates as the denominator, exercise 20%, exactly 30%, 31%, handled 31%, 31% to 30% to 31%, and continuous above-threshold edits. Confirm no retired checklist row or interaction is present. |
...
10. Open the PDF and inspect headings, checklist applicability, statistics, finding severity and resolution, status, page numbers, privacy footer, wrapping, and the absence of clipping or overlap. Confirm the retired Arrival Month checklist row is absent, File Month Statistics remain present, and the Arrival Month statistics Failure appears under Failed Checks only above 30%.
